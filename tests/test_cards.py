...
async def test_create_cards(
    ...
    assert response.status_code == status_codes.HTTP_201_CREATED, response.text
    ...
    assert data["extra"][0]["message"] == "Unique constraint violated for Card"
    assert data["extra"][0]["key"] == "deck_id, front"
